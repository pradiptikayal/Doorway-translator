      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "room_status") {
            ...
            if (msg.languageA) {
              setLangA(msg.languageA);
            }
            if (msg.languageB) {
              setLangB(msg.languageB);
            }
                  ...
                  {utterances.length === 0 ? (
                    ...
                    utterances.map((utterance) => (
                      <div key={utterance.id} className="mb-4 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
                        <p className="text-xs uppercase tracking-widest text-stone-400">Spoken ({myActiveLanguage})</p>
                        <p className="mt-1 text-sm text-stone-800">{utterance.userText || "Listening..."}</p>
                        <p className="mt-4 text-xs uppercase tracking-widest text-stone-400 font-semibold text-stone-600">Translated ({partnerActiveLanguage})</p>
